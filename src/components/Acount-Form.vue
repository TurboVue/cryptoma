<template>
    <div class="flex items-center h-full justify-center">
        <div id="overlay"><form class="form">
            <div class="input_box">
                <label
                    for="bank"
                    class="font-medium"
                >Account Bank</label>
                    ...
                    >
                        <div
                            @click="toggleList"
                            class="w-full input border-grey-400 border text-tiny capitalize px-4 py-4 rounded-sm focus:border-cyan-500 focus:shadow-outline outline-none outline-none"
                        >{{ details[0].bank }} </div>

            <ul
                v-if="isShow"
                class="codes rounded border bg-white w-full p-2 my-2  overflow-scroll"
            >
                <li
                    class="px-2 py-3 hover:bg-gray-100 flex items-center relative cursor-pointer"
    ...
    <button class="mx-2 px-4 py-2 cursor-pointer text-tiny rounded text-white bg-cyan-500 focus:outline-none hover:bg-cyan-400" @click.prevent="checkPay()">Save</button>
    </form></div>
